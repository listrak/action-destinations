...
export const getAdvertiserAudiences = async (
  ...
  if (response.status !== 200)
    // Centrifuge will automatically retry the batch if there's
    // an issue fetching the Advertiser's audiences from Listrak.
...
export const getAudienceId = async (
  ...
  try {
    ...
    if (e instanceof ListrakAPIError) {
...
export const createAudience = async (
  ...
  if (response.status !== 200) {
    ...
    throw new ListrakAPIError(`Error while creating the Audience`, 'Listrak audience creation error', 400, err)
  }
