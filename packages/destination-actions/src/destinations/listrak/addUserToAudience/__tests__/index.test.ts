...
describe('addUserToAudience', () => {
  it('should throw error if no access to the audiences of the advertiser', async () => {
    ...
    nock('https://api.listrak.com').persist().post('/oauth2/token').reply(200, MOCK_TOKEN_RESPONSE)
    nock('https://api.listrak.com').get(/^\/\d{4}-\d{2}\/audiences$/).query({ "advertiser-id": settings.advertiser_id }).reply(403)
    await expect(
  ...
  it('should not throw an error if the audience creation and the patch requests succeed', async () => {
    const settings = VALID_SETTINGS;
    nock('https://api.listrak.com').persist().post('/oauth2/token').reply(200, MOCK_TOKEN_RESPONSE)
    nock('https://api.listrak.com').get(/^\/\d{4}-\d{2}\/audiences$/).query({ "advertiser-id": settings.advertiser_id }).reply(200, {
      "data": [
        {
          "id": "5678",
          "attributes": {
    ...
    nock('https://api.listrak.com').post(/^\/\d{4}-\d{2}\/audiences$/).reply(200, AUDIENCE_CREATION_RESPONSE)
    nock('https://api.listrak.com').patch(/^\/\d{4}-\d{2}\/audiences\/\d+\/contactlist$/).reply(200)
  ...
  it('should not throw an error if the audience already exists and the patch requests succeeds', async () => {
    const settings = VALID_SETTINGS;
    nock('https://api.listrak.com').persist().post('/oauth2/token').reply(200, MOCK_TOKEN_RESPONSE)
    nock('https://api.listrak.com').get(/^\/\d{4}-\d{2}\/audiences$/).query({ "advertiser-id": settings.advertiser_id }).reply(200, ADVERTISER_AUDIENCES)
    nock('https://api.listrak.com').patch(/^\/\d{4}-\d{2}\/audiences\/\d+\/contactlist$/).reply(200)
  ...
  it('should not throw an error in case of concurrent audience creation attempt', async () => {
    const settings = VALID_SETTINGS;
    nock('https://api.listrak.com').persist().post('/oauth2/token').reply(200, MOCK_TOKEN_RESPONSE)
    nock('https://api.listrak.com').persist().get(/^\/\d{4}-\d{2}\/audiences$/).query({ "advertiser-id": settings.advertiser_id }).reply(200, {
      "data": [
        {
          "id": "5678",
          "attributes": {
    ...
    nock('https://api.listrak.com').post(/^\/\d{4}-\d{2}\/audiences$/).reply(400, DUPLICATE_AUDIENCE_ERROR)
    nock('https://api.listrak.com').patch(/^\/\d{4}-\d{2}\/audiences\/\d+\/contactlist$/).reply(200)
